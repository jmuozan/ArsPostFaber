...
class MeshEditor:
    ...
    def __init__(self, mesh_path="cylinder.stl"):
        ...
        self._device_index = getattr(self, '_device_index', 0)
        print(f"Using camera device index: {self._device_index}")
        self.camera = cv2.VideoCapture(self._device_index)
        if not self.camera.isOpened():
            print(f"Warning: Could not open camera with device index {self._device_index}!")
    ...
    def show_help(self):
        ...
        print("Help overlay activated - hold mouse button to keep visible")
    ...
    def process_mouse_event(self, event, x, y, flags, param):
        ...
        # Handle mouse wheel for zooming
            ...
            self.view_controller.scale(zoom_factor)
            self.update_visualization()
        elif event == cv2.EVENT_MOUSEHWHEEL:  # macOS support
            delta = -np.sign(flags)
            zoom_factor = 1.1 if delta > 0 else 0.9
            self.view_controller.scale(zoom_factor)
            self.update_visualization()
...
def main():
    ...
    try:
        ...
        # Set device index if it's an attribute
        if hasattr(editor, '_device_index'):
